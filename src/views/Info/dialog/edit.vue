...
<el-dialog  title="编辑"  :visible.sync="dialog_info_flag" @close="close"  width="580px" @opened="category">
...
import {EdidInfo,GetList} from '@/api/news.js'
  export default {
    name:'dialogEdit',
    props:{
            flag:{
    ...
    setup(props,{emit,root,refs}){
      ...
      const getList=()=>{
          ...
          GetList(resqustData).then(Response=>{
     ...
     const submit=()=>{
       ...
       EdidInfo(data).then(Response=>{
         root.$message({
           message:Response.data.message,
           type:'success'
         })
         emit('resposeInfo',data);
         loading.value=false;
         //重置表单
        //  root.$nextTick();
       refs['form'].resetFields(); 
